...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("source"); ap.add_argument("markdown"); ap.add_argument("intent"); a=ap.parse_args()
    ...
    records=[]
    for item in params: records.append({"schema":"t2c.intent/v1","id":hashlib.sha256(("param:"+item["name"]+digest).encode()).hexdigest()[:16],"type":"claim","text":f"SCAD parameter {item['name']} = {item['value']}","actor":"scad-parser","targetUris":[f"urn:subactor:resource:sha256:{digest}"],"source":{"artifactUri":f"urn:subactor:resource:sha256:{digest}","revisionHash":digest,"fragment":item["name"],"converter":"scad-source","converterVersion":"1"}})
    for dep in includes: records.append({"schema":"t2c.intent/v1","id":hashlib.sha256(("dep:"+dep+digest).encode()).hexdigest()[:16],"type":"claim","text":f"SCAD dependency use <{dep}>","actor":"scad-parser","targetUris":[f"urn:subactor:resource:sha256:{digest}"],"source":{"artifactUri":f"urn:subactor:resource:sha256:{digest}","revisionHash":digest,"fragment":dep,"converter":"scad-source","converterVersion":"1"}})
    records.append({"schema":"t2c.intent/v1","id":hashlib.sha256(("geometry:"+digest).encode()).hexdigest()[:16],"type":"claim","text":f"SCAD geometry operators: {', '.join(primitives) or 'none'}; modules: {', '.join(modules) or 'none'}; source lines: {len(text.splitlines())}","actor":"scad-parser","targetUris":[f"urn:subactor:resource:sha256:{digest}"],"source":{"artifactUri":f"urn:subactor:resource:sha256:{digest}","revisionHash":digest,"fragment":"geometry","converter":"scad-source","converterVersion":"1"}})
