...
test("dashboard serves the live twin, scene and USD, and applies intake durably", async (t) => {
  ...
  assert.match(dashboardHtml, /Project MD/);
  assert.match(dashboardHtml, /Project HTML/);
  assert.match(dashboardHtml, /Project PDF/);
  assert.match(dashboardHtml, /\/api\/documentation\?format=manifest/);
  ...
  const documentationMarkdownResponse=await fetch(`${server.url}api/documentation?format=md`);
  assert.equal(documentationMarkdownResponse.status,200);
  assert.match(documentationMarkdownResponse.headers.get("content-type")??"",/^text\/markdown/);
  assert.match(documentationMarkdownResponse.headers.get("content-disposition")??"",/attachment; filename="dashboard-factory-project-documentation\.md"/);
  assert.match(documentationMarkdownResponse.headers.get("x-subactor-project-documentation-uri")??"",/^urn:subactor:project-documentation:sha256:/);
  assert.equal(documentationMarkdownResponse.headers.get("x-subactor-twin-revision-uri"),state.active.revisionUri);
  assert.match(await documentationMarkdownResponse.text(),/# Digital Twin project — Dashboard Factory/);
  const documentationHtmlResponse=await fetch(`${server.url}api/documentation?format=html`);
  assert.equal(documentationHtmlResponse.status,200);
  assert.match(await documentationHtmlResponse.text(),/^<!doctype html>/);
  const documentationPdfResponse=await fetch(`${server.url}api/documentation?format=pdf`);
  assert.equal(documentationPdfResponse.status,200);
  assert.match(documentationPdfResponse.headers.get("content-type")??"",/^application\/pdf/);
  assert.equal(Buffer.from(await documentationPdfResponse.arrayBuffer()).subarray(0,8).toString("ascii"),"%PDF-1.4");
  const documentationManifest=await (await fetch(`${server.url}api/documentation?format=manifest`)).json() as {schema:string;activeRevision:{twinUri:string};artifacts:Record<string,{sha256:string}>};
  assert.equal(documentationManifest.schema,"subactor.project-documentation-manifest/v1");
  assert.equal(documentationManifest.activeRevision.twinUri,state.active.revisionUri);
  assert.ok(documentationManifest.artifacts["project-documentation.pdf"].sha256);
  assert.equal((await fetch(`${server.url}api/documentation?format=docx`)).status,400);
