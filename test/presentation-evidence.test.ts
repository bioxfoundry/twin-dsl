...
test("presentation evidence distinguishes missing, unverified, current, stale and tampered captures",async()=>{
  ...
  try {
    ...
    assert.match(renderPresentationEvidenceDsl(current),/CAMERA MODE STATIC EYE \[8, 6, 5\]/);
...
test("presentation manifest rejects traversal, duplicate paths and unknown fields",()=>{
  const base={schema:"subactor.presentation-evidence/v1",twinUri:contentUri("twin",twin),sceneUri:contentUri("scene",scene),capturedAt:"2026-08-10T00:00:00Z",renderer:{name:"dashboard",version:"1"},captures:[{path:"overview.png",sha256:"a".repeat(64),bytes:1,mediaType:"image/png",camera:{mode:"static",eye:[8,6,5],target:[0,0,0],up:[0,0,1],verticalFovDeg:45,trajectorySha256:null}}]};
