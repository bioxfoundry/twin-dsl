---
schema: bioxfoundry.error-page/v1
code: PHYSICAL_EVIDENCE_REJECTED
source: error/catalog.json
generated: true
---

# PHYSICAL_EVIDENCE_REJECTED — Physical evidence rejected

- Subsystem: `physical evidence`
- Severity: `error`
- Error class: `policy`
- Retryable: `false`
- Surfaces: `response`

## Meaning

The physical evidence operation was refused by an explicit safety or authority boundary.

## Likely causes

- the requested action is outside the allowed policy
- required approval, grounding or mutation authority is absent

## Impact

The operation is not applied; existing accepted state is preserved.

## Resolution

Do not bypass the boundary. Correct the request or provide the required scoped authority and repeat validation.

## Emitted by

- `src/serve/dashboard.ts`
