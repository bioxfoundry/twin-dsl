...
export function validatePresentationEvidence(value:unknown):PresentationEvidenceManifest {
  ...
  const captures=document.captures.map((value,index)=>{
    ...
    exact(capture,["path","sha256","bytes","mediaType"],"PRESENTATION_CAPTURE");
    ...
    return {path:capture.path,sha256:capture.sha256,bytes:capture.bytes,mediaType:capture.mediaType} as PresentationCapture;
  });
...
async function capturesIn(directory:string):Promise<PresentationCapture[]> {
  ...
  for(const path of names) {
    ...
    captures.push({path,sha256:sha256(bytes),bytes:bytes.length,mediaType:path.toLowerCase().endsWith(".png")?"image/png":"video/webm"});
  }
  return captures;
}
...
export async function inspectPresentationEvidence(directory:string,twin:TwinDocument,scene:SceneDocument):Promise<PresentationEvidenceSummary> {
  ...
  return finish(problems.length?"stale":"current",actualCaptures,problems);
}
...
export function renderPresentationEvidenceDsl(summary:PresentationEvidenceSummary):string {
  ...
  for(const capture of summary.captures) lines.push(`CAPTURE ${quote(capture.path)} SHA256 ${capture.sha256} BYTES ${capture.bytes} MEDIA ${capture.mediaType}`);
