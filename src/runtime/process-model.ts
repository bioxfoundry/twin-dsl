...
export function deriveBiofoundryProcesses(input: {
  ...
  const mosAxesXY = deviceEvidence(DEVICE_ARTIFACTS.mos3sSyringeFirmware, "MOS3S XY axis envelope", ["x_bed_size 190", "y_bed_size 190"]);
  const mosAxesZ = deviceEvidence(DEVICE_ARTIFACTS.mos3sSyringeFirmware, "MOS3S Z axis envelope", ["z_max_pos 200", "software endstops"]);
  ...
  if ([mosMachine, mosSafety, mosAxesXY, mosAxesZ, mosHoming, mosJob].some(Boolean)) {
    ...
    const steps = [
      ...
      step({ id: "position_printhead", label: "Position carriage and platform inside the firmware travel envelope", phase: "operate", componentIds: motionGroup, interactions: [interaction("operation", motionGroup)], transitions: { success: "deposit_dual_material" }, evidence: [mosAxesXY, mosAxesZ].filter((item): item is ProcessEvidence => Boolean(item)), parameters: parameters([parameter(mosAxesXY, "x_travel", 190, "mm"), parameter(mosAxesXY, "y_travel", 190, "mm"), parameter(mosAxesZ, "z_travel", 200, "mm")]) }),
