...
export function renderProjectDsl(doc:LivingProjectDocument):string{
  ...
  out.push(`POLICY_APPROVED ${doc.policy.approved}`,`POLICY_REQUIRE_RESEARCH ${doc.policy.requireResearch}`,`POLICY_REQUIRE_DEVELOPMENT ${doc.policy.requireDevelopmentEvidence}`,`POLICY_REQUIRE_DEVELOPMENT_ACCEPTANCE ${doc.policy.requireDevelopmentAcceptance}`,`POLICY_ALLOW_DEVELOPMENT_FIXTURE ${doc.policy.allowDevelopmentFixture}`,`POLICY_REQUIRE_RUNTIME ${doc.policy.requireRuntimeEvidence}`,`POLICY_AUTO_PUBLISH_SCENE ${doc.policy.autoPublishScene}`,`POLICY_ALLOW_RUNTIME_SELF_MODIFICATION ${doc.policy.allowRuntimeSelfModification}`,`POLICY_AUTONOMY_MODE ${doc.policy.autonomyMode}`,`POLICY_REQUIRE_SIGNED_MUTATION_GRANT ${doc.policy.requireSignedMutationGrant}`,...(doc.policy.mutationGrantFile?[`POLICY_MUTATION_GRANT_FILE ${JSON.stringify(doc.policy.mutationGrantFile)}`]:[]),`POLICY_MAX_ITERATIONS_PER_HOUR ${doc.policy.maxIterationsPerHour}`,`POLICY_MAX_CONSECUTIVE_FAILURES ${doc.policy.maxConsecutiveFailures}`,`SCENE_FORMAT ${doc.scene.format}`,...(doc.scene.blueprintFile?[`SCENE_BLUEPRINT_FILE ${JSON.stringify(doc.scene.blueprintFile)}`]:[]),...(doc.scene.physicalEvidenceFile?[`SCENE_PHYSICAL_EVIDENCE_FILE ${JSON.stringify(doc.scene.physicalEvidenceFile)}`]:[]));return out.join('\n')+'\n';
}
