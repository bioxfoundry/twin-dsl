...
function readme(name:string,runtimeVersion:string):string { return `# ${name} — Living Digital Twin

Generated by Subactor Digital Twin Runtime Starter ${runtimeVersion}.
...
async function vendorRuntime(target:string):Promise<string> {
  const here = dirname(fileURLToPath(import.meta.url));
  const root = resolve(here,"../../..");
  const runtimePackage=JSON.parse(await readFile(join(root,"package.json"),"utf8")) as {version?:unknown};
  ...
  await cp(join(root,"deploy/runtime/Dockerfile"),join(vendor,"Dockerfile"));
...
`);
  const runtimeVersion=await vendorRuntime(projectDir);
  await text(join(projectDir,"README.md"),readme(options.name,runtimeVersion));
